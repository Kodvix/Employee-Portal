...
useEffect(() => {
  const fetchEmployees = async () => {
    try {
      const response = await fetch('http://192.168.1.32:8080/api/employee');
      if (response.ok) {
...
const handleUpdateRecord = async () => {
  ...
  try {
    const response = await fetch(`http://192.168.1.32:8080/api/attendances/${selectedRecord.id}`, {
      method: 'PUT',
...
const handleCreateRecord = async () => {
  ...
  try {
    const response = await fetch('http://192.168.1.32:8080/api/attendances', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
const handleDeleteRecord = async (recordId) => {
  ...
  try {
    const response = await fetch(`http://192.168.1.32:8080/api/attendances/${recordId}`, {
      method: 'DELETE',
    });
