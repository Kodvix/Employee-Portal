...
const AdminDocumentsPage = () => {
  ...
  const fetchEmployeeDocuments = async (employeeId) => {
    ...
    try {
      ...
      // Ensure company data is an array, handle single object response
      const companyData = Array.isArray(companyResponse.data) ? companyResponse.data : (companyResponse.data ? [companyResponse.data] : []);
      
      // Process company documents to include mime types and names
      const processedCompanyDocs = companyData.map(doc => {
          let name = 'Company Document';
          let mimeType = 'application/octet-stream'; // Default
          let base64 = null;

          if (doc.offerLetterDoc) { name = 'Offer Letter'; mimeType = 'application/pdf'; base64 = doc.offerLetterDoc; }
          else if (doc.latestPaySlipDoc) { name = 'Latest Payslip'; mimeType = 'application/pdf'; base64 = doc.latestPaySlipDoc; }
          else if (doc.doc) { 
              name = 'Additional Document'; 
              // Assuming additional docs might have a type field or default to PDF
              mimeType = doc.type || 'application/pdf'; 
              base64 = doc.doc;
          }

          return { ...doc, name, mimeType, base64Doc: base64 };
      }).filter(doc => doc.base64Doc != null);
      ...
      const personalResponse = await axios.get(`${API_BASE_URL}/documents/employee/${employeeId}`);
       console.log('Admin - Personal documents response:', personalResponse.data);
        // Ensure personal data is an array
      const personalData = Array.isArray(personalResponse.data) ? personalResponse.data : [];

       // Process personal documents to include mime types
      const processedPersonalDocs = personalData.map(doc => {
          // Assuming personal docs have type and data fields
           const mimeType = doc.type || 'application/octet-stream';
           console.log(`Processing personal doc ${doc.name}, MIME: ${mimeType}, Base64 length: ${doc.data?.length || 0}`);
           if (!doc.data) console.warn(`No base64 data for personal doc ${doc.name}`);
          return { ...doc, mimeType, base64Doc: doc.data };
      }).filter(doc => doc.base64Doc != null);
